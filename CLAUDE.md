...
- `widgets/` — Qt::Widgets controls, depends on `canfan::core`
  ...
  - `window/framelesswidget` — frameless window: 8-direction edge resize anywhere (5px default margin, `setResizeMargin()`), but window move + double-click maximize ONLY on watched panels (TitleBar) since v0.10.0 — presses on content areas (control bars, list blanks) must not drag the window. Prefers native `startSystemMove`/`startSystemResize` (Windows snap, Wayland), falls back to manual implementation when the platform doesn't take over.
  - `window/framelesshandler` — the event-filter object that implements the above behavior (extracted so `FramelessDialog` shares it). The window-level filter handles edge resize only; `watch(panel)` routes a panel's mouse events through move/resize/dblclick logic with panel→window coordinate translation, consuming them so they never bubble to the window (double-handling); construct with `filterTarget=false` to get a watch-only carrier (TitleBar does this — do NOT let a second handler also filter the window, double-click maximize would toggle twice)
