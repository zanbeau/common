...
qmake is supported alongside CMake: `common.pri` is the consumer entry point (compiles sources into the including project, TTKCommon-style), `common.pro` builds a static lib standalone, `examples/widgets_demo.pro` builds the demo. When adding a file, update `common.pri` (HEADERS/SOURCES) as well as the CMake lists.
...
`build-test/` is configured with the Visual Studio 17 2022 generator (open `build-test/common.sln`, or `cmake --build build-test`). Tests live in `tests/` (`tst_widgets`, run on the offscreen QPA platform); examples in `examples/` (`widgets_demo`). Tests and examples only build when `common` is the top-level project. No lint/format tooling is configured. clangd is used (`.cache/clangd`; `compile_commands.json` lives in `build/`).
