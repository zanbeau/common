...
- `widgets/` — Qt::Widgets controls, depends on `canfan::core`
  ...
  - `window/titlebar` — caption bar (title + min/max/close) for frameless windows; itself and the title label are `WA_TransparentForMouseEvents` so press/double-click fall through to the window's FramelessHandler (drag / maximize), only the three CaptionButtons handle clicks; CaptionButton lives in the .cpp with Q_OBJECT + `#include "titlebar.moc"` (AUTOMOC picks it up); max/restore glyph switched via eventFilter on WindowStateChange
